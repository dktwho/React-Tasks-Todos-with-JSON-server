...
import { List, AddList, Tasks } from './components/dotAllImport';
  ...
  const onRemoveTask = (listId, taskId) => {
    if (window.confirm("Вы действительно хотите удалить задачу?")) {
      ...
      axios.delete(`http://localhost:3001/tasks/${taskId}`).catch(() => {
        console.log("error delete task todo");
      });
    }
  };
  ...
  const onCompletedTask = (listId, taskId, completed) => {
    if (window.confirm("Вы действительно хотите изменить задачу?")) {
      ...
      axios.patch(`http://localhost:3001/tasks/${taskId}`, {completed}).catch(() => {
        console.log("error completed task todo");
      });
    }
      ...
      <div className="todo__sidebar">
        ...
        <div className="todo__tasks">
          <Routes>
            ...
            <Route exact path="/lists/:id" element={lists && activeItem &&  <Tasks onCompletedTask={onCompletedTask} onRemoveTask={onRemoveTask} onEditTask={onEditTask} list={activeItem} onEditTitle={onEditListTitle} onAddTask={onAddTask} />}></Route>
          </Routes>
        </div>
    </div>
  );
